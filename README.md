...
If a custom Maven Repository is required, the S2i images also supports it, in case the **MAVEN_REPO_URL** environment variable
is provided a new Repository and Plugin Repository will be added, if no repo-id is provided using the **MAVEN_REPO_ID** environment
variable a generated one will be used. There is also the possibility to provide more than one custom Repository, in this case, 
we need to provide the repo **prefix** using the **MAVEN_REPOS** environment variable, example, if we want to add two new repositories
