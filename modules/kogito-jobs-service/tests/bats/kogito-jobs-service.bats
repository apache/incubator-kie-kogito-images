...
@test "check if the event is correctly set" {
    ...
    export KAFKA_BOOSTRAP_SERVERS="localhost:9999"
    configure_jobs_service

    result="${KOGITO_JOBS_PROPS}"
    expected=" -Dquarkus.profile=events-support -Dmp.messaging.outgoing.kogito-job-service-job-status-events.bootstrap.servers=${KAFKA_BOOSTRAP_SERVERS} -Devents-support.quarkus.kafka.bootstrap-servers=${KAFKA_BOOSTRAP_SERVERS}"
