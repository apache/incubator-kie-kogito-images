...
function configure_proxy() {
    ...
    if [ -n "$proxyhost" ]; then
        ...
        sed -i.bak -e "s|<!-- ### configured http proxy ### -->|${xml}|" "${MAVEN_SETTINGS_PATH}"
    fi
}
...
function configure_mirrors() {
    if [ -n "$MAVEN_MIRROR_URL" ]; then
      ...
      <mirrorOf>external:*</mirrorOf>\
        ...
        sed -i.bak -e "s|<!-- ### configured mirrors ### -->|$xml|" "${MAVEN_SETTINGS_PATH}"
    fi
}
...
function set_kogito_maven_repo() {
    ...
    if [ -n "${kogito_maven_repo_url}" ]; then
        sed -i.bak -e "s|https://repository.jboss.org/nexus/content/groups/public/|${kogito_maven_repo_url}|" "${MAVEN_SETTINGS_PATH}"
    fi
}
...
function _add_maven_repo() {
    ...
    sed -i.bak -e "s|<!-- ### configured repositories ### -->|${repo}|" "${MAVEN_SETTINGS_PATH}"
    ...
    sed -i.bak -e "s|<!-- ### configured plugin repositories ### -->|${pluginRepo}|" "${MAVEN_SETTINGS_PATH}"

    # new repo should be skipped by mirror if exists
    sed -i.bak -e "s|</mirrorOf>|,!${repo_id}</mirrorOf>|g" "${MAVEN_SETTINGS_PATH}"
}
