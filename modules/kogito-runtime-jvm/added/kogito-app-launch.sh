...
case ${runtime_type} in 
    "quarkus") 
        exec java ${JAVA_OPTS} ${JAVA_OPTIONS} ${KOGITO_QUARKUS_JVM_PROPS} ${CUSTOM_TRUSTSTORE_ARGS} \
            -Dquarkus.http.host=0.0.0.0 \
            -Dquarkus.http.port=8080 \
            -jar "${KOGITO_HOME}"/bin/*.jar
    ;;
    "springboot") 
        exec java ${JAVA_OPTS} ${JAVA_OPTIONS} ${KOGITO_SPRINGBOOT_PROPS} ${CUSTOM_TRUSTSTORE_ARGS} \
