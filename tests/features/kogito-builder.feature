...
Feature: kogito-builder image tests
  ...
  Scenario: Verify that the Kogito Maven archetype is generating the project and compiling it correctly
    Given s2i build /tmp/kogito-examples from dmn-example using nightly-1.14.x and runtime-image quay.io/kiegroup/kogito-runtime-jvm:latest
      | variable       | value          |
      | RUNTIME_TYPE   | quarkus        |
      | NATIVE         | false          |
  ...
  Scenario: Verify that the Kogito Quarkus Serverless Workflow Extension is building the service properly
    Given s2i build /tmp/kogito-examples from serverless-workflow-order-processing/src/main/resources using nightly-1.14.x and runtime-image quay.io/kiegroup/kogito-runtime-jvm:latest
      | variable            | value                 |
      | RUNTIME_TYPE        | quarkus               |
      | NATIVE              | false                 |
      | KOGITO_VERSION | 1.14.0-SNAPSHOT | 
