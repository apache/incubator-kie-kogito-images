...
Feature: Kogito-trusty feature.
  ...
  Scenario: verify if all labels are correctly set.
    ...
    And the image should contain label io.k8s.description with value Runtime image for Trusty Index Service
  ...
  Scenario: Verify if the debug is correctly enabled and test default http port
    When container is started with env
      | variable     | value |
      | SCRIPT_DEBUG | true  |
    Then container log should contain + exec java -XshowSettings:properties -Dquarkus.infinispan-client.use-auth=false -Dquarkus.http.port=8080 -Djava.library.path=/home/kogito/lib -Dquarkus.http.host=0.0.0.0 -jar /home/kogito/bin/kogito-trusty-runner.jar
  ...
  Scenario: Verify if the debug is correctly enabled and test custom http port
    When container is started with env
      ...
      | SCRIPT_DEBUG  | true  |
      | HTTP_PORT     | 9090  |
    Then container log should contain + exec java -XshowSettings:properties -Dquarkus.infinispan-client.use-auth=false -Dquarkus.http.port=9090 -Djava.library.path=/home/kogito/lib -Dquarkus.http.host=0.0.0.0 -jar /home/kogito/bin/kogito-trusty-runner.jar

  Scenario: Verify trusty default configuration
    When container is started with env
      | variable     | value |
      | SCRIPT_DEBUG | true  |
